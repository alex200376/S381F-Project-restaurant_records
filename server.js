...
const updateDocument = async(db, criteria, updatedDocument) => {
    const result = await db.collection("restaurants").updateOne(criteria, updatedDocument);
    return result;
};
...
app.post("/create", requireAuthentication, async(req, res) => {
    try {
        ...
        if (result.insertedCount === 1) {
            setTimeout(() => {
                res.send(`<script>alert('Restaurant created successfully!'); window.location.href='/details?_id=${createdDocument._id}';</script>`);
            }, 2000);
        } else {
            setTimeout(() => {
                res.send(`<script>alert('Failed to create restaurant.'); window.location.href='/create';</script>`);
            }, 2000);
        }
    } catch (err) {
        console.error(err);
        res.send(`<script>alert('An error occurred while creating the restaurant.'); window.location.href='/create';</script>`);
    }
});
...
app.post("/update", requireAuthentication, async(req, res) => {
    try {
        ...
        if (result && result.modifiedCount === 1) {
            setTimeout(() => {
                res.send(`<script>alert('Restaurant updated successfully!'); window.location.href='/details?_id=${req.body.postId}';</script>`);
            }, 2000);

        } else {
            setTimeout(() => {
                res.send(`<script>alert('Restaurant update failed.'); window.location.href='/details?_id=${req.body.postId}';</script>`);
            }, 2000);

        }
    } catch (err) {
        console.error(err);
        res.send(`<script>alert('An error occurred while updating the restaurant.'); window.location.href='/details?_id=${req.body.postId}';</script>`);
    }
});
app.get("/delete", requireAuthentication, async(req, res) => {
    ...
    if (result.deletedCount > 0) {
        setTimeout(() => {
            res.send(`<script>alert('Restaurant deleted successfully!'); window.location.href='/home';</script>`);
        }, 2000);

    } else {
        setTimeout(() => {
            res.send(`<script>alert('Restaurant delete failed.'); window.location.href='/home';</script>`);
        }, 2000);
